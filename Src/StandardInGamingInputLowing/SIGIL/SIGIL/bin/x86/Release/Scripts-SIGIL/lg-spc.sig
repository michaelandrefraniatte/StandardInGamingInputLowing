...
namespace StringToCode
{
    ...
    { 
        ...
        private static SwitchProController spc = new SwitchProController();
        ...
        private void task()
        {
            for (; ; )
            {
                if (!running)
                    break;
                valchanged(0, spc.ProControllerButtonCAPTURE);
                ...
                if (getstate[0]) 
                {
                    ...
                    mousexp += Math.Round(spc.ProControllerGyroX * 65535f / 2f / 1024f * 2f / 20000f, 0) * 2f;
                    mouseyp += Math.Round(spc.ProControllerGyroY * 65535f / 2f / 1024f * 2f / 20000f, 0) * 2f;
                    if (mousexp >= width / 2f) 
                        mousexp = width / 2f;
                    if (mousexp <= -width / 2f) 
                        mousexp = -width / 2f;
                    ...
                    if (mouseyp <= -height / 2f) 
                        mouseyp = -height / 2f;
                    MouseDesktopX  = width / 2f - mousexp - Math.Round(spc.ProControllerGyroX * 65535f / 2f / 1024f * 2f / 20000f, 0) * 2f;
                    MouseDesktopY  = height / 2f + mouseyp + Math.Round(spc.ProControllerGyroY * 65535f / 2f / 1024f * 2f / 20000f, 0) * 2f;
                    SendD          = spc.ProControllerLeftStickX > 0.35f;
                    SendQ          = spc.ProControllerLeftStickX < -0.35f;
                    SendZ          = spc.ProControllerLeftStickY > 0.35f;
                    SendS          = spc.ProControllerLeftStickY < -0.35f;
                    Send8          = spc.ProControllerButtonDPAD_DOWN;
                    Send7          = spc.ProControllerButtonDPAD_LEFT;
                    Send9          = spc.ProControllerButtonDPAD_RIGHT;
                    Send6          = spc.ProControllerButtonDPAD_UP;
                    SendSpace      = spc.ProControllerButtonSHOULDER_Left_1;
                    SendLeftShift  = spc.ProControllerButtonSHOULDER_Right_1;
                    SendE          = spc.ProControllerButtonMINUS;
                    SendB          = spc.ProControllerButtonSTICK_Right;  
                    SendR          = spc.ProControllerButtonY;
                    SendF          = spc.ProControllerButtonX;
                    SendX          = spc.ProControllerButtonB;
                    SendC          = spc.ProControllerButtonA;
                    SendRightClick = spc.ProControllerButtonSHOULDER_Left_2;
                    SendLeftClick  = spc.ProControllerButtonSHOULDER_Right_2;
                    SendH          = spc.ProControllerButtonSTICK_Left;
                    SendEscape     = spc.ProControllerButtonHOME;
                }
