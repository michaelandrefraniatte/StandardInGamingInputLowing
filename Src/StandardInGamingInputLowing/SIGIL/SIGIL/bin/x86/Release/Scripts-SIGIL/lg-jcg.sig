    ...
    { 
        ...
        private void task()
        {
            for (; ; )
            {
                if (!running)
                    break;
                valchanged(0, jcg.JoyconRightButtonPLUS);
                if (wd[0] == 1 & !getstate)  
                {
                ...
                if (getstate) 
                {
                    ...
                    mousexp += Math.Round(jcg.JoyconRightGyroX * width / 2f / 1024f * 2f / 200f, 0) * 1f;
                    mouseyp += Math.Round(jcg.JoyconRightGyroY * height / 2f / 1024f * 2f / 200f, 0) * 1f;
                    if (mousexp >= width / 2f) 
                        mousexp = width / 2f;
                    if (mousexp <= -width / 2f) 
                        mousexp = -width / 2f;
                    ...
                    if (mouseyp <= -height / 2f) 
                        mouseyp = -height / 2f;
                    MouseDesktopX  = width / 2f + mousexp + Math.Round(jcg.JoyconRightGyroX * width / 2f / 1024f * 2f / 200f, 0) * 1f;
                    MouseDesktopY  = height / 2f - mouseyp - Math.Round(jcg.JoyconRightGyroY * height / 2f / 1024f * 2f / 200f, 0) * 1f;
                    SendD          = jcg.JoyconLeftStickX > 0.25f;
                    SendQ          = jcg.JoyconLeftStickX < -0.25f;
                    SendZ          = jcg.JoyconLeftStickY > 0.25f;
                    SendS          = jcg.JoyconLeftStickY < -0.25f;
                    Send5          = jcg.JoyconRightStickX > 0.25f;
                    Send6          = jcg.JoyconRightStickX < -0.25f;
                    Send3          = jcg.JoyconRightStickY > 0.25f;
                    Send4          = jcg.JoyconRightStickY < -0.25f;
                    Send7          = jcg.JoyconLeftButtonDPAD_UP;
                    Send8          = jcg.JoyconLeftButtonDPAD_LEFT;
                    Send9          = jcg.JoyconLeftButtonDPAD_DOWN;
                    Send0          = jcg.JoyconLeftButtonDPAD_RIGHT;
                    SendT          = jcg.JoyconLeftButtonMINUS;
                    SendEscape     = jcg.JoyconLeftButtonCAPTURE;
                    SendLeftShift  = jcg.JoyconLeftButtonSTICK;
                    SendA          = jcg.JoyconLeftButtonSL;
                    SendLMENU      = jcg.JoyconLeftButtonSR;
                    SendSpace      = jcg.JoyconLeftButtonSHOULDER_1;
                    Send1          = jcg.JoyconLeftButtonSHOULDER_2;
                    SendC          = jcg.JoyconRightButtonDPAD_DOWN;
                    Send2          = jcg.JoyconRightButtonDPAD_RIGHT;
                    SendRightClick = jcg.JoyconRightButtonDPAD_LEFT;
                    SendX          = jcg.JoyconRightButtonDPAD_UP;
                    SendG          = jcg.JoyconRightButtonPLUS;
                    SendF          = jcg.JoyconRightButtonHOME;
                    SendV          = jcg.JoyconRightButtonSTICK;
                    SendTab        = jcg.JoyconRightButtonSL;
                    SendE          = jcg.JoyconRightButtonSR;
                    SendR          = jcg.JoyconRightButtonSHOULDER_1;
                    SendLeftClick  = jcg.JoyconRightButtonSHOULDER_2;
                }
