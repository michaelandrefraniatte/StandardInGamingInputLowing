...
{
    /// <summary>
    /// Interaction logic for DualShock4 controllers.
    /// </summary>
    public class DualShock4
    {
        ...
        private DualShock4(IDevice underlyingDevice, int? writeBufferSize)
        {
        ...
        /// expected that you will only use the <see cref="DualShock4"/> instance passed as an argument to 
        ...
        /// <returns>A parsed <see cref="DualShock4Touch"/>.</returns>
        ...
        public static DualShock4 EnumerateControllers(string vendor_id, string product_id, string label_id)
        {
            foreach (ConnectedDeviceDefinition deviceDefinition in ListDevices(vendor_id, product_id, label_id))
            {
                ...
                return new DualShock4(device, deviceDefinition.WriteBufferSize);
            }
            return null;
        }

        /// <summary>
        /// A handler for a state polling IO event. The sender has the <see cref="DualShock4InputState"/>
        /// from the most recent poll, and can be used to update the next
        /// <see cref="DualShock4OutputState"/>.
        /// </summary>
        /// <param name="sender">The <see cref="DualShock4"/> instance that was just polled.</param>
