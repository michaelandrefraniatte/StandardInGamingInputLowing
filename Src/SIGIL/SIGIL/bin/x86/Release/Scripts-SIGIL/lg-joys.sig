    ...
    { 
        ...
        private void task()
        {
            for (; ; )
            {
                if (!running)
                    break;
                valchanged(0, jr.JoyconRightButtonPLUS);
                ...
                if (getstate[0]) 
                {
                    ...
                    mousexp += Math.Round(jr.JoyconRightGyroX * width / 2f / 1024f * 2f / 200f, 0) * 1f;
                    mouseyp += Math.Round(jr.JoyconRightGyroY * height / 2f / 1024f * 2f / 200f, 0) * 1f;
                    if (mousexp >= width / 2f) 
                        mousexp = width / 2f;
                    if (mousexp <= -width / 2f) 
                        mousexp = -width / 2f;
                    ...
                    if (mouseyp <= -height / 2f) 
                        mouseyp = -height / 2f;
                    MouseDesktopX  = width / 2f + mousexp + Math.Round(jr.JoyconRightGyroX * width / 2f / 1024f * 2f / 200f, 0) * 1f;
                    MouseDesktopY  = height / 2f - mouseyp - Math.Round(jr.JoyconRightGyroY * height / 2f / 1024f * 2f / 200f, 0) * 1f;
                    SendD          = jl.JoyconLeftStickX > 0.25f;
                    SendQ          = jl.JoyconLeftStickX < -0.25f;
                    SendZ          = jl.JoyconLeftStickY > 0.25f;
                    SendS          = jl.JoyconLeftStickY < -0.25f;
                    Send5          = jr.JoyconRightStickX > 0.25f;
                    Send6          = jr.JoyconRightStickX < -0.25f;
                    Send3          = jr.JoyconRightStickY > 0.25f;
                    Send4          = jr.JoyconRightStickY < -0.25f;
                    Send7          = jl.JoyconLeftButtonDPAD_UP;
                    Send8          = jl.JoyconLeftButtonDPAD_LEFT;
                    Send9          = jl.JoyconLeftButtonDPAD_DOWN;
                    Send0          = jl.JoyconLeftButtonDPAD_RIGHT;
                    SendT          = jl.JoyconLeftButtonMINUS;
                    SendEscape     = jl.JoyconLeftButtonCAPTURE;
                    SendLeftShift  = jl.JoyconLeftButtonSTICK;
                    SendA          = jl.JoyconLeftButtonSL;
                    SendLMENU      = jl.JoyconLeftButtonSR;
                    SendSpace      = jl.JoyconLeftButtonSHOULDER_1;
                    Send1          = jl.JoyconLeftButtonSHOULDER_2;
                    SendC          = jr.JoyconRightButtonDPAD_DOWN;
                    Send2          = jr.JoyconRightButtonDPAD_RIGHT;
                    SendRightClick = jr.JoyconRightButtonDPAD_LEFT;
                    SendX          = jr.JoyconRightButtonDPAD_UP;
                    SendG          = jr.JoyconRightButtonPLUS;
                    SendF          = jr.JoyconRightButtonHOME;
                    SendV          = jr.JoyconRightButtonSTICK;
                    SendTab        = jr.JoyconRightButtonSL;
                    SendE          = jr.JoyconRightButtonSR;
                    SendR          = jr.JoyconRightButtonSHOULDER_1;
                    SendLeftClick  = jr.JoyconRightButtonSHOULDER_2;
                }
