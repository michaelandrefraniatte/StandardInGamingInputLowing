...
namespace StringToCode
{
    ...
    { 
        ...
        private GamepadsHooks gh = new GamepadsHooks();
        ...
        private void task()
        {
            for (; ; )
            {
                if (!running)
                    break;
                valchanged(0, gh.ControllerButtonBackPressed);
                ...
                if (getstate[0]) 
                {
                    statex = Math.Abs(gh.ControllerThumbRightX / 32767f * limit) <= limit ? gh.ControllerThumbRightX / 32767f * limit : Math.Sign(gh.ControllerThumbRightX) * limit;
                    statey = Math.Abs(gh.ControllerThumbRightY / 32767f * limit) <= limit ? gh.ControllerThumbRightY / 32767f * limit : Math.Sign(gh.ControllerThumbRightY) * limit;
                    ...
                    if (statey <= (dzy / 100f) * limit & statey >= -(dzy / 100f) * limit)
                        mousey = 0f;
                    MouseMoveX     = Math.Sign(gh.ControllerThumbRightX) * (Math.Abs(statex * statex * statex) / limit / limit * viewpower3x + Math.Abs(statex * statex) / limit * viewpower2x + Math.Abs(statex) * viewpower1x);
                    MouseMoveY     = -Math.Sign(gh.ControllerThumbRightY) * (Math.Abs(statey * statey * statey) / limit / limit * viewpower3y + Math.Abs(statey * statey) / limit * viewpower2y + Math.Abs(statey) * viewpower1y);
                    SendD          = gh.ControllerThumbLeftX > 32767f / 5f;
                    SendQ          = gh.ControllerThumbLeftX < -32767f / 5f;
                    SendZ          = gh.ControllerThumbLeftY > 32767f / 5f;
                    SendS          = gh.ControllerThumbLeftY < -32767f / 5f;
                    Send0          = gh.ControllerButtonDownPressed;
                    Send1          = gh.ControllerButtonLeftPressed;
                    Send2          = gh.ControllerButtonRightPressed;
                    Send3          = gh.ControllerButtonUpPressed;
                    SendSpace      = gh.ControllerButtonShoulderLeftPressed;
                    SendLeftShift  = gh.ControllerButtonShoulderRightPressed;
                    SendB          = gh.ControllerThumbpadRightPressed;  
                    SendR          = gh.ControllerButtonXPressed;
                    SendF          = gh.ControllerButtonYPressed;
                    SendX          = gh.ControllerButtonAPressed;
                    SendC          = gh.ControllerButtonBPressed;
                    SendRightClick = gh.ControllerTriggerLeftPosition >= 250f;
                    SendLeftClick  = gh.ControllerTriggerRightPosition >= 250f;
                    SendH          = gh.ControllerThumbpadLeftPressed;
                    SendEscape     = gh.ControllerButtonStartPressed;
                }
