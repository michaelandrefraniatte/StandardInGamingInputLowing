...
namespace Readdevices
{
    public class ReadDevice : IDisposable
    {
        [DllImport("hid.dll")]
        ...
        public void Scan(string vendor_id, string product_id, int length, int number = 0)
        {
            this.number = number;
            this.length = length;
            this.mBuff = new byte[length];
            this.aBuffer = new byte[length];
            if (number <= 1)
            {
                int index = 0;
                Guid guid;
        ...
        private bool Found(string path)
        {
            try
            {
                ...
                mStream = new FileStream(handle, FileAccess.Read, length, true);
                return true;
            }
            catch { return false; }
        }
