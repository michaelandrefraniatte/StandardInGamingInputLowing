...
extern "C"
{
	__declspec(dllexport) void hid_read_timeout(hid_device* dev, unsigned char* data, size_t length)
	{
		ReadFile(dev->device_handle, dev->read_buf, dev->input_report_length, &bytes_read, NULL);
		memcpy(data, dev->read_buf, length);
	}
	__declspec(dllexport) void hid_write(hid_device* dev, const unsigned char* data, size_t length)
	{
